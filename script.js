// --- MUSIC AND OVERLAY LOGIC ---
const startButton = document.getElementById('startButton');
const overlay = document.getElementById('overlay');
const bgMusic = document.getElementById('bgMusic');

startButton.addEventListener('click', () => {
    bgMusic.play(); // Starts the music
    overlay.style.opacity = '0'; // Fades out
    setTimeout(() => {
        overlay.style.display = 'none'; // Removes from view
    }, 1000);
});
// ------------------------------
...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    const serviceID = "service_x6gcfbg";
    const templateID = "template_o651hdi";
...
function drawText() {
    ...
    context.shadowColor = "rgb(139, 0, 204)";
    context.shadowBlur = 8;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;

    if(frameNumber < 250){
        context.fillStyle = `rgba(139, 0, 204, ${opacity})`;
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        context.fillStyle = `rgba(139, 0, 204, ${opacity})`;
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Emily, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Emily, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
        opacity = opacity + 0.01;
    }
    
    if(frameNumber >= 2750 && frameNumber < 99999){
        context.fillStyle = `rgba(139, 0, 204, ${secondOpacity})`;
        if (window.innerWidth < 600) {
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        context.fillStyle = `rgba(139, 0, 204, ${thirdOpacity})`;
